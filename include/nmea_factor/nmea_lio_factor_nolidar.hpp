...
#include <nmea_factor/integration_base.hpp>

namespace ligo {

class NmeaLioFactorNolidar
    : public gtsam::NoiseModelFactor4<gtsam::Rot3, gtsam::Vector12, gtsam::Rot3, gtsam::Vector12> {
 public:
  NmeaLioFactorNolidar(gtsam::Key i1, gtsam::Key i2, gtsam::Key j1, gtsam::Key j2, gtsam::Rot3 rel_rot_,
                       gtsam::Point3 rel_pos_, gtsam::Vector3 rel_vel_, gtsam::Vector3 grav_, double dt_,
                       gtsam::Vector3 bias_ai_, gtsam::Vector3 bias_gi_, IntegrationBase *pre_integration_,
      ...
      : gtsam::NoiseModelFactor4<gtsam::Rot3, gtsam::Vector12, gtsam::Rot3, gtsam::Vector12>(model, i1, i2, j1, j2),
        ...
        grav(grav_),
        dt(dt_),
        linearized_ba(bias_ai_),
        linearized_bg(bias_gi_),
        pre_integration(pre_integration_) {}

  virtual ~NmeaLioFactorNolidar() {}
  gtsam::Vector evaluateError(const gtsam::Rot3 &rot1, const gtsam::Vector12 &pos_vel_bias1, const gtsam::Rot3 &rot2,
                              const gtsam::Vector12 &pos_vel_bias2, gtsam::OptionalMatrixType H1 = OptionalNone,
                              gtsam::OptionalMatrixType H2 = OptionalNone, gtsam::OptionalMatrixType H3 = OptionalNone,
                              gtsam::OptionalMatrixType H4 = OptionalNone) const override {
    Eigen::Matrix3d d = rot1.transpose() * rot2.matrix();
    Eigen::Vector3d delta_p =
        rot1.transpose() *
        (pos_vel_bias2.segment<3>(0) - pos_vel_bias1.segment<3>(0) - pos_vel_bias1.segment<3>(3) * dt - 0.5 * grav * dt * dt);
    ...
    Eigen::Vector3d dba = pos_vel_bias1.segment<3>(6) - linearized_ba;
    Eigen::Vector3d dbg = pos_vel_bias1.segment<3>(9) - linearized_bg;
    ...
    if (H1) {
      ...
      (*H1).block<3, 3>(3, 0) = -Jacob_right_inv<double>(res_r) * d.transpose();
    ...
    if (H2) {
      ...
      (*H2).block<3, 3>(0, 0) = -rot1.transpose();
      (*H2).block<3, 3>(0, 3) = -rot1.transpose() * dt;
      (*H2).block<3, 3>(6, 3) = -rot1.transpose();
      (*H2).block<3, 3>(3, 9) = -Jacob_right_inv<double>(res_r) * corrected_delta_q.transpose() * d.matrix() * dq_dbg;
      (*H2).block<3, 3>(0, 6) = -dp_dba;
      (*H2).block<3, 3>(0, 9) = -dp_dbg;
      (*H2).block<3, 3>(6, 6) = -dv_dba;
      (*H2).block<3, 3>(6, 9) = -dv_dbg;
    ...
    if (H3) {
      ...
      (*H3).block<3, 3>(3, 0) = Jacob_right_inv<double>(res_r);
      (*H3) = sqrt_info * (*H3);
    }
    if (H4) {
