...
from navi.msg import NmeaHeadingAlignStatus
try:
...
class LigoMqttBridge(Node):
    ...
    def _publish_position(self, *, force: bool = False) -> None:
        if not self.nmea_enable:
            ...
            if not force:
                ...
                if (
                    self._last_local_position_publish_unix > 0.0
                    and (now - self._last_local_position_publish_unix)
                    < self._local_position_min_interval_sec
                ):
                    return
                self._last_local_position_publish_unix = now
            self._publish_json(
